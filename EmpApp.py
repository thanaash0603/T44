...
@app.route("/showleaveForm", methods=['GET', 'POST'])
def showleaveform():
    db_conn = connections.Connection(
        host=customhost,
        port=3306,
...
@app.route("/showsummaryLeave", methods=['GET', 'POST'])
def showsummaryleave():
    db_conn = connections.Connection(
        host=customhost,
        port=3306,
        user=customuser,
    ...
    cursor = db_conn.cursor()
    cursor.execute("select * from leaveForm")
    data = cursor.fetchall()  # data from database
...
def Addleave():
    ...
    insert_sql = "INSERT INTO leaveForm VALUES (%s, %s, %s, %s, %s, %s)"
    cursor = db_conn.cursor()
    if emp_image_file.filename == "":
       return "Please select a file"
